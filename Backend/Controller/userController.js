...
const JWT_SECRET = process.env.JWT_SECRET || 'Khv';
...
export async function loginUser(req, res) {
    ...
    if (!email || !password) {
        return res.status(400).json({ success: false, message: "Email and password requires" });
    }
...
export async function updateProfile(req, res) {
    ...
    try {
        ...
        if (exists) {
            return res.status(409).json({ success: false, message: "Email already exist, in use with other account" });
        }
